...
1. Clone this repository with `$ git clone https://github.com/aarondill/TStemplate <name-for-your-folder>`
...
```bash
read -rep 'What to call your folder? ' folderName && [[ -n "$folderName" ]] && git clone https://github.com/aarondill/TStemplate "$folderName" && cd "$folderName" && ./init
```
